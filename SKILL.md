# Parametric 3D Printing with CadQuery — Complete Guide

This comprehensive resource covers using CadQuery (Python-based CAD) to generate 3D-printable parametric models exported as STL files.
...
- **Fillet/shell/cut ordering**: The correct sequence is ".shell() first (hollow out), then .fillet() (round edges), then boolean cuts"
...
- **Taper direction**: In ".extrude(taper=angle), a positive taper angle narrows the shape (draft inward)"
- **Shell on tapered/complex geometry**: ".shell() frequently fails on tapered bodies, lofted shapes, or geometry with many fillets"
- **Loft is fragile**: CadQuery's ".loft() fails on many cross-section combinations"
